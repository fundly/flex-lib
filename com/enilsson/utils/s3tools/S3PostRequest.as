...
package com.enilsson.utils.s3tools 
{
    ...
    public class S3PostRequest extends EventDispatcher 
    {
        ...
        private function buildPostVariables():URLVariables 
        {
            ...
            if(_options.successActionRedirect != null) {
                postVariables.success_action_redirect = _options.successActionRedirect;
            }
        ...
        private function onHttpStatus(event:HTTPStatusEvent):void 
        {
            ...
            {
                this.dispatchEvent(
                	new DataEvent(DataEvent.UPLOAD_COMPLETE_DATA, event.bubbles, event.cancelable, "Amazon S3 returned HTTP status " + event.status.toString() + ".")
                );
            }
            else
            {
                this.dispatchEvent(
                	new IOErrorEvent(IOErrorEvent.IO_ERROR, event.bubbles, event.cancelable, "Amazon S3 returned an error: HTTP status " + event.status.toString() + ".")
                );
            }
        }
